...
for mov in opening_movies['movies']:
    ...
    for i, actor in enumerate( mov['abridged_cast'] ):
        ...
        if i < l:
            cast += ', '
    movie = media.Movie(mov['title'], mov['synopsis'], mov['posters']['thumbnail'], mov['posters']['detailed'], mov['mpaa_rating'], mov['release_dates']['theater'], mov['ratings']['critics_score'], cast)
